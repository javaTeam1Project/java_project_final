...
public class publicity_board_update extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					publicity_board_update window = new publicity_board_update();
	...
	public publicity_board_update() { // 매개변수로 받아온 아이디값 활용
	...
	private void initialize() {
		...
		btdel.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				output = dao.read_publicity_board1(idstr); // 특정 아이디로 게시글 검색 컨트롤러 호출 이때 매개변수로 아이디값 넘겨줌
				int row = table.getSelectedRow();
				tableModel = (DefaultTableModel) table.getModel();
				tableModel.removeRow(row);
				dao.delete_publicity_board(output.get(row).getId());
				output.remove(row);
		...
		btsearch.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				output = dao.search_publicity_board(title);
				...
				System.out.println(dao.search_publicity_board("안녕").get(0).getTitle());
		...
		udbtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				output = dao.read_publicity_board1(idstr);
				int row = table.getSelectedRow();
				tableModel = (DefaultTableModel) table.getModel();
				...
				if (row >= 0) {
					...
					dao.update_publicity_board(output.get(row).getId(), output.get(row).getTitle(),
