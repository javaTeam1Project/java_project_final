...
public class team_board_update extends JFrame {
	team_board_controller dao = new team_board_controller(); // 데이터베이스 연동을 위해 컨트롤러 생성 필수
	public JFrame frame;
	private JTextArea textArea;
	private JTable table;
	private DefaultTableModel tableModel;
	ArrayList<team_board> output = new ArrayList<team_board>();
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					team_board_update window = new team_board_update();
	 ...
	 */
	public team_board_update() { // 매개변수로 받아온 아이디값 활용
		initialize(); // 매개변수로 아이디 넘겨줌
	}
	...
	private void initialize() {
		...
		btdel.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				output = dao.read_team_board1(idstr); // 특정 아이디로 게시글 검색 컨트롤러 호출 이때 매개변수로 아이디값 넘겨줌
				int row = table.getSelectedRow();
				tableModel = (DefaultTableModel) table.getModel();
				tableModel.removeRow(row);
				dao.delete_team_board(output.get(row).getId());
				output.remove(row);
		...
		btsearch.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				output = dao.search_team_board(title);
				int size = output.size();
				String data[][] = new String[size][4];

				for (int i = 0; i < output.size(); i++) {
					String User = Integer.toString(output.get(i).getId());
				...
				System.out.println(dao.search_team_board("안녕").get(0).getTitle());
		...
		udbtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				output = dao.read_team_board1(idstr);
				int row = table.getSelectedRow();
				tableModel = (DefaultTableModel) table.getModel();
				...
				if (row >= 0) {
					...
					dao.update_team_board(output.get(row).getId(), output.get(row).getTitle(),
