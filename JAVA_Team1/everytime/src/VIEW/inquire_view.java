...
public class inquire_view extends JFrame {
	inquire_controller dao = new inquire_controller();
	ArrayList<inquire> output = new ArrayList<inquire>();
	private DefaultTableModel model;
	String idstr = LoginUser.getInstance().getId();

	
	public inquire_view(String idstr) {
		setTitle("문의사항");

		inquire_controller dao = new inquire_controller();
		String columnNames[] = { "작성자", "제목", "내용", "작성일" };
		output = dao.read_inquire();
		...
		JLabel EveryDongEui = new JLabel("문의사항");
		...
		JButton bt_write = new JButton("질문남기기");
		South.add(ch2);
		South.add(search);
		South.add(bt_search);
		South.add(bt_write);
		...
		bt_write.setPreferredSize(new Dimension(120, 30));
		...
		bt_search.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				output = dao.search_inquire_board(checksearch, title);
				int size = output.size();
				String data[][] = new String[size][4];
