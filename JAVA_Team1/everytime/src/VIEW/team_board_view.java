...
public class team_board_view extends JFrame {
	team_board_controller dao = new team_board_controller(); // 데이터베이스 연동을 위해 컨트롤러 생성 필수
	ArrayList<team_board> output = new ArrayList<team_board>(); // 데이터베이스에서 가져온 값 모음
	DefaultTableModel model;
	JTable table;
	String idstr = LoginUser.getInstance().getId();

	
	public team_board_view(int num) { // 매개변수로 로그인된 아이디 받아옴
		setTitle("팀원모집게시판");

		String columnNames[] = { "작성자", "제목", "내용", "작성일" };

		output = dao.search_team_post(num); // 컨트롤러 연동해서 데이터베이스에 접근하여 데이터 삽입, 삭제, 조회, 검색, 수정 가능
		...
		for (int i = 0; i < output.size(); i++) {
			String user = output.get(i).getUser(); // 데이터베이스에서 값을 가져오면 DTO 객체를 생성해서 해당 DTO에 들어있는 변수에 값이 저장됨
		...
		JLabel EveryDongEui = new JLabel("팀원모집게시판");
		...
		bt_search.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				output = dao.search_team_board(checksearch, title);
		...
		bt_update.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				team_board_update a = new team_board_update(); // 로그인된 아이디 매개변수 넘겨줌
